...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  // Global Minimize Dashboard State & Section Collapse States
  const [isMinimized, setIsMinimized] = useState<boolean>(() => {
    try {
      return localStorage.getItem('focusflow_dashboard_minimized') === 'true';
    } catch (e) {
      return false;
    }
  });
  ...
  const [collapsedSections, setCollapsedSections] = useState<Record<string, boolean>>(() => {
    try {
      const saved = localStorage.getItem('focusflow_dashboard_collapsed_sections');
      if (saved) {
        const parsed = JSON.parse(saved);
        return { ...parsed, banner: true };
      }
    } catch (e) {}
    return {
      banner: true,
  ...
  useEffect(() => {
    try {
      localStorage.setItem('focusflow_dashboard_minimized', isMinimized ? 'true' : 'false');
    } catch (e) {}
  }, [isMinimized]);

  useEffect(() => {
    try {
      localStorage.setItem('focusflow_dashboard_collapsed_sections', JSON.stringify(collapsedSections));
    } catch (e) {}
  }, [collapsedSections]);
  ...
  return (
    <div className="space-y-4 sm:space-y-6 pb-8 animate-in fade-in duration-300">
      {/* Minimize Dashboard Control Toolbar */}
      <div className="p-3 sm:p-3.5 rounded-2xl bg-white/80 dark:bg-[#2B2C31]/80 amoled:bg-[#121315]/80 backdrop-blur-xl border border-slate-200/80 dark:border-white/10 shadow-xs flex flex-wrap items-center justify-between gap-2.5">
        <div className="flex items-center gap-2">
          <button
            onClick={() => setIsMinimized(!isMinimized)}
            className={`px-3 py-1.5 rounded-xl font-extrabold text-xs flex items-center gap-2 transition-all cursor-pointer shadow-2xs ${
              isMinimized
                ? 'bg-[#7FAF8E] text-white hover:bg-[#639272]'
                : 'bg-slate-100 hover:bg-slate-200 dark:bg-white/10 dark:hover:bg-white/15 text-[#2F3A45] dark:text-slate-200'
            }`}
          >
            {isMinimized ? (
              <>
          ...
          <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold border ${
            isMinimized
              ...
              : 'bg-[#7FAF8E]/15 text-[#639272] dark:text-[#7FAF8E] border-[#7FAF8E]/30'
          }`}>
            {isMinimized ? 'COMPACT FOCUS MODE' : 'FULL DASHBOARD'}
          </span>
        </div>

        {/* Section Quick Visibility Pills */}
        <div className="flex items-center gap-1.5 overflow-x-auto max-w-full py-0.5 scrollbar-none">
          <span className="text-[10px] font-extrabold text-[#9CA3AF] uppercase tracking-wider hidden sm:inline mr-1">
            Cards:
          </span>
          {[
            ...
            const isCollapsed = isMinimized || collapsedSections[sec.key];
            return (
              <button
                key={sec.key}
                ...
                className={`px-2 py-0.5 rounded-lg text-[10px] font-bold transition-all cursor-pointer flex items-center gap-1 ${
                  isCollapsed
                    ? 'bg-slate-200/60 dark:bg-white/5 text-[#9CA3AF] line-through'
                    : 'bg-[#7FAF8E]/20 text-[#639272] dark:text-[#7FAF8E]'
                }`}
                title={`Click to ${isCollapsed ? 'expand' : 'collapse'} ${sec.label}`}
              >
                <span>{sec.label}</span>
          ...
          {(isMinimized || Object.values(collapsedSections).some(Boolean)) && (
            <button
              onClick={expandAllSections}
              className="text-[10px] font-extrabold text-[#7FAF8E] hover:underline px-1.5 cursor-pointer ml-1"
            >
              Expand All
            </button>
          )}
      ...
      <AnimatePresence>
        {(!collapsedSections.banner) && (
          <motion.div
            ...
            className={`bento-card relative overflow-hidden bg-gradient-to-br from-[#EBF4EE] via-[#FAFAF8] to-[#F0F5FA] dark:from-[#1E1F22] dark:via-[#23252A] dark:to-[#2B2C31] text-[#2F3A45] dark:text-white border border-[#7FAF8E]/30 shadow-md max-w-full transition-all ${
              isMinimized ? 'p-3 sm:p-4' : 'p-4 sm:p-6 lg:p-8'
            }`}
          >
            ...
            {/* Banner Action Controls: Delete / Hide & Collapse */}
            <div className="absolute top-2.5 right-2.5 flex items-center gap-1.5 z-20">
              <button
                onClick={() => toggleSectionCollapse('banner')}
                className="p-1.5 rounded-xl bg-white/60 dark:bg-white/10 hover:bg-white text-[#9CA3AF] hover:text-[#2F3A45] dark:hover:text-white transition-all cursor-pointer"
            ...
            {isMinimized ? (
              /* Minimized Compact Banner Bar */
              <div className="relative z-10 flex flex-wrap items-center justify-between gap-3 pr-8">
                <div className="flex items-center gap-2.5">
                  <span className="w-7 h-7 rounded-xl bg-[#7FAF8E]/20 text-[#7FAF8E] flex items-center justify-center font-bold text-xs">
                    <Sun className="w-4 h-4" />
                  </span>
                  <div>
                    <h1 className="text-sm font-extrabold text-[#2F3A45] dark:text-white">
                      {greeting}, Alex.
                    </h1>
                    <p className="text-[11px] text-[#9CA3AF] font-medium">
                      {new Date().toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })} • {stats.currentStreak} Day Streak
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-[#7FAF8E] hover:bg-[#639272] text-white font-bold text-xs flex items-center gap-1.5 shadow-2xs cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-white/80 dark:bg-white/10 text-[#2F3A45] dark:text-white font-bold text-xs border border-slate-200/80 dark:border-white/10 cursor-pointer"
                  >
              ...
              <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-5 sm:gap-6 w-full max-w-full min-w-0 pr-6">
      ...
      <AnimatePresence>
        {!collapsedSections.metrics && (
          ...
          >
            {isMinimized ? (
              /* Minimized Compact Metrics Ticker */
              <div className="bento-card p-3 flex flex-wrap items-center justify-between gap-3 text-xs bg-white/60 dark:bg-white/5 border border-slate-200/60 dark:border-white/10">
                <div className="flex items-center gap-2">
                  <span className="font-extrabold text-[#7FAF8E]">Progress:</span>
                  <span className="font-bold text-[#2F3A45] dark:text-slate-200">{completedToday}/{totalToday} Tasks ({progressPercent}%)</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="font-extrabold text-[#A7C7E7]">Next:</span>
                  <span className="font-bold text-[#2F3A45] dark:text-slate-200 truncate max-w-[150px]">{nextEvent ? nextEvent.title : 'None'}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="font-extrabold text-[#C8B6E2]">Focus Hours:</span>
                  <span className="font-bold text-[#2F3A45] dark:text-slate-200">{stats.focusHoursToday}h</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="font-extrabold text-[#7FAF8E]">Zen Score:</span>
                  <span className="font-bold text-[#2F3A45] dark:text-slate-200">{stats.productivityScore}%</span>
                </div>
                <button
                  onClick={() => toggleSectionCollapse('metrics')}
                  className="p-1 text-[#9CA3AF] hover:text-[#2F3A45] dark:hover:text-white cursor-pointer ml-auto"
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-white/5">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-white/10 dark:hover:bg-white/15 text-[#9CA3AF] hover:text-[#2F3A45] dark:hover:text-white transition-all cursor-pointer"
              title={collapsedSections.goals ? 'Expand Section' : 'Collapse Section'}
            >
              {collapsedSections.goals ? <ChevronDown className="w-4 h-4" /> : <ChevronUp className="w-4 h-4" />}
            </button>
          </div>
        </div>
        ...
        <div className="bento-card p-[21px] space-y-5">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-white/5">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => toggleSectionCollapse('timeline')}
                className="p-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-white/10 dark:hover:bg-white/15 text-[#9CA3AF] hover:text-[#2F3A45] dark:hover:text-white transition-all cursor-pointer"
                title={collapsedSections.timeline ? 'Expand Timeline' : 'Collapse Timeline'}
              >
          ...
          <div className="bento-card p-6 space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-white/5">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button
                  onClick={() => toggleSectionCollapse('habits')}
                  className="p-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-white/10 dark:hover:bg-white/15 text-[#9CA3AF] hover:text-[#2F3A45] dark:hover:text-white transition-all cursor-pointer"
                  title={collapsedSections.habits ? 'Expand Habits' : 'Collapse Habits'}
                >
